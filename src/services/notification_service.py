...
class NotificationService:
    def __init__(
        self, repository: NotificationsRepository, template_service: TemplateService
    ) -> None:
    ...
    async def get_existing_idempotent_notification(
        ...
        if payload.idempotency_key is None:
            raise RuntimeError(
                "Cannot resolve idempotency race without idempotency key"
            )
        ...
        if existing is None:
            raise RuntimeError(
                "Idempotency race happened, but existing notification was not found"
            )
    ...
    def _is_same_request(
        notification: Notification, payload: NotificationCreate
    ) -> bool:
        return (
    ...
    async def prepare_send(
    ...
    ) -> tuple[Notification, TemplateRenderResponse]:
        ...
        rendered = await self.template_service.render_template(
            notification.template_code, notification.context
        )
